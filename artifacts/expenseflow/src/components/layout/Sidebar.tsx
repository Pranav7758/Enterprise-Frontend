    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-4 border-b border-white/5">
        <div className="flex items-center gap-2.5 overflow-hidden">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-purple-500 to-purple-700 flex items-center justify-center shrink-0 shadow-lg shadow-purple-500/30">
          ...
          <AnimatePresence initial={false}>
            {!collapsed && (
              <motion.span
                ...
                className="font-bold text-white text-sm tracking-tight whitespace-nowrap overflow-hidden"
              >
                ExpenseFlow
              </motion.span>
            )}
        ...
        {/* Toggle button — always visible, inside the sidebar */}
        <button
          onClick={onToggle}
          className="flex items-center justify-center w-7 h-7 rounded-lg bg-white/8 hover:bg-purple-500/20 border border-white/10 hover:border-purple-500/40 text-muted-foreground hover:text-purple-300 transition-all duration-200 shrink-0"
      ...
      {/* Org + role badge */}
      <div className={cn(
        "flex items-center gap-2.5 mx-2 my-2.5 px-2.5 py-2.5 rounded-xl",
                ...
                <AnimatePresence initial={false}>
                  {!collapsed && (
                    <motion.p
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="text-[9px] uppercase tracking-[0.12em] text-muted-foreground/50 px-3 pt-3 pb-1 font-bold"
                    >
                      {group.title}
                    </motion.p>
                  )}
                    ...
                    return (
                      <li key={item.href}>
                        <Link href={item.href}>
                          <a
                            className={cn(
                              "group flex items-center gap-3 rounded-xl text-sm font-medium transition-all duration-150 cursor-pointer relative",
      ...
      <div className="border-t border-white/5 p-2">
        <div className={cn(
          "flex items-center gap-2.5 px-2 py-2 rounded-xl hover:bg-white/5 transition-colors",
          collapsed ? "justify-center" : ""
        )}>
          {/* Avatar */}
          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-purple-500 to-cyan-500 flex items-center justify-center shrink-0 text-white text-[11px] font-bold shadow">
            {initials}
          </div>
